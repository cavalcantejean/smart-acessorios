...
import type { Comment } from "@/lib/types"; // Original Comment type with Timestamp
...
// import { addCommentAccessoryAction } from "@/app/accessory/[id]/actions"; // Passed as prop
import { useToast } from "@/hooks/use-toast";
import Link from "next/link";
import { Timestamp } from "firebase/firestore";

// Type for comment as it's handled/displayed in this client component
interface ClientComment extends Omit<Comment, 'createdAt'> {
  createdAt: string; // ISO string
}

interface CommentsSectionProps {
  accessoryId: string;
  comments: ClientComment[]; // Expects comments with string dates
  onCommentAdded: (newComment: ClientComment) => void;
  serverAddCommentAction: (prevState: any, formData: FormData) => Promise<any>; // Server action passed as prop
}
...
interface CommentActionResult {
  success: boolean;
  comment?: ClientComment; // Server action should return comment with string date
  message?: string;
  error?: string;
...
export default function CommentsSection({ accessoryId, comments: initialComments, onCommentAdded, serverAddCommentAction }: CommentsSectionProps) {
  const { user, isAuthenticated, isLoading: isLoadingAuth } = useAuth();
  const [state, formAction, isPending] = useActionState(serverAddCommentAction, initialActionState);
  ...
  const onSubmit = (data: CommentFormValues) => {
    ...
    formData.append("userName", user.name || "Usuário Anônimo");
    startTransition(() => {
      formAction(formData);
    });
  };
  ...
  return (
    <div className="space-y-6 pt-6 border-t mt-6">
      ...
      {isAuthenticated ? (
        <Form {...form}>
          <form ref={formRef} action={formAction as any} onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <FormField
            ...
            {user && <input type="hidden" name="userName" value={user.name || "Usuário Anônimo"} />}
